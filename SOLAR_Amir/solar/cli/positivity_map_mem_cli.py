"""CLI for generating positivity probability maps from membrane channel images.

Processes all image files in a given folder.

Examples:
    python -m solar.cli.positivity_map_mem_cli /path/to/membrane_folder /path/to/output
    python -m solar.cli.positivity_map_mem_cli /path/to/membrane_folder /path/to/output --tile_free
    python -m solar.cli.positivity_map_mem_cli /path/to/membrane_folder /path/to/output --debug --tile_size 256 --tile_overlap 64
"""
...
def main():
    """Parse arguments, load all images from folder, compute positivity maps, and write outputs."""
    parser = argparse.ArgumentParser(description="Generate positivity probability maps for all files in a membrane channel folder.")
    parser.add_argument("input_folder", type=str, help="Path to the folder containing membrane channel images.")
    parser.add_argument("output_folder", type=str, help="Path to the folder for saving output positivity maps.")
    ...
    if not os.path.isdir(args.input_folder):
        raise ValueError(f"Input folder not found: {args.input_folder}")

    os.makedirs(args.output_folder, exist_ok=True)

    # Get all image files in the folder
    image_files = [
        name for name in os.listdir(args.input_folder)
        if name.lower().endswith((".tif", ".tiff", ".npy"))
    ]

    if not image_files:
        raise ValueError(f"No image files found in: {args.input_folder}")

    print(f"Processing {len(image_files)} files from {args.input_folder}")

    for filename in sorted(image_files):
        input_path = os.path.join(args.input_folder, filename)
        base_name = os.path.splitext(filename)[0]
        output_filename = f"{base_name}_positivity_map.tiff"
