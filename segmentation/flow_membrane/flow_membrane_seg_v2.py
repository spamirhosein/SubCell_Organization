#!/usr/bin/env python3
"""
flow_membrane_seg_v2.py
=======================
v2 of flow_membrane_seg.py. Two changes:
  * tuned defaults  : veto_wall = 0 (merging off), flow_weight = 0.8
  * new final step  : irregular masks are deleted (branched / sliver / fragment)
Everything else is unchanged, so v1 output is reproducible with
...
Knobs (all exposed in `Params`):
    ...
    min_circularity  4*pi*A/P^2 below this => not a cell (catches branched sprawl).
    min_thickness    2*max(distance transform)/equiv. diameter below this => sliver.
    min_area         masks smaller than this (px) are dropped at the end.
...
class Params:
    ...
    flow_weight: float = 0.8    # weight of the flow-separatrix term in the elevation
    veto_wall: float = 0.0      # merge if shared-border mean-membrane < this; 0 = no merging
    hole_ratio: float = 0.99    # delete a mask if real area < hole-filled area

    # --- irregular-mask filter (v2). Set any to 0 to disable that test ---
    min_circularity: float = 0.22   # 4*pi*A/P^2; low for branched / tentacle shapes
    min_thickness: float = 0.45     # 2*max(dt)/equiv_diameter; low for slivers
    min_area: int = 800             # drop fragments
...
# --------------------------------------------------------------------------- #
# Delete masks whose shape is not cell-like  (v2)
# --------------------------------------------------------------------------- #
def shape_metrics(region) -> tuple:
    """(circularity, thickness) for one regionprops region.

    circularity = 4*pi*A/P^2      -> 1 for a disc, falls with total perimeter, so it
                                     catches branched / tentacle-like masks.
    thickness   = 2*max(dt) / equivalent diameter
                                  -> 1 for a disc, near 0 for a thin sliver. Built on
                                     the distance transform, so unlike circularity it
                                     is not thrown off by a ragged outline.
    The two fail on different shapes, which is why both are used.
    """
    per = region.perimeter if region.perimeter > 0 else 1.0
    circularity = 4 * np.pi * region.area / per ** 2
    dt = ndi.distance_transform_edt(np.pad(region.image, 1))
    equiv_diameter = 2 * np.sqrt(region.area / np.pi)
    thickness = 2 * dt.max() / max(equiv_diameter, 1e-9)
    return float(circularity), float(thickness)


def remove_irregular(labels: np.ndarray, p: Params):
    """Delete masks that are branched, sliver-like, or too small.

    Returns (labels, n_removed). Tests with a threshold of 0 are skipped.
    """
    if p.min_circularity <= 0 and p.min_thickness <= 0 and p.min_area <= 0:
        return labels, 0

    bad = []
    for r in regionprops(labels):
        circularity, thickness = shape_metrics(r)
        if ((p.min_circularity > 0 and circularity < p.min_circularity)
                or (p.min_thickness > 0 and thickness < p.min_thickness)
                or (p.min_area > 0 and r.area < p.min_area)):
            bad.append(r.label)

    out = labels.copy()
    if bad:
        out[np.isin(labels, np.asarray(bad, dtype=labels.dtype))] = 0
        out, _, _ = relabel_sequential(out)
    return out, len(bad)
...
def run_pipeline(seg: dict, p: Params = Params()):
    ...
    labels = remove_holey(labels, p)      # delete masks with holes
    n_pre_shape = int(labels.max())
    labels, n_irregular = remove_irregular(labels, p)   # v2: delete non-cell shapes
    ...
    ctx = dict(nucleus=nucleus, membrane=membrane, nuc_plane=nuc_plane,
               ...
               n_before_shape_filter=n_pre_shape, n_irregular=n_irregular)
    return labels, ctx
...
def process_one(seg_path: str, out_dir: str, p: Params, overlay: bool):
    ...
    print(f"[{name}] sinks={ctx['n_markers']}  cells={ctx['n_cells']} "
          f"(Cellpose={base})  irregular_removed={ctx['n_irregular']}  "
          f"low_conf={n_low}  nucleus=plane{ctx['nuc_plane']}")
    return labels, rows


def main():
    ap = argparse.ArgumentParser(description="Flow + membrane hybrid segmentation (v2).")
    ...
    ap.add_argument("--min-circularity", type=float, default=Params.min_circularity,
                    help="0 disables the circularity test")
    ap.add_argument("--min-thickness", type=float, default=Params.min_thickness,
                    help="0 disables the thickness test")
    ap.add_argument("--min-area", type=int, default=Params.min_area,
                    help="0 disables the area test")
    args = ap.parse_args()

    p = Params(sink_h=args.sink_h, flow_weight=args.flow_weight, veto_wall=args.veto_wall,
               min_circularity=args.min_circularity, min_thickness=args.min_thickness,
